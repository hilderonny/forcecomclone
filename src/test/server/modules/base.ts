...
describe.only('Module base', () => {
    ...
    describe.only('API login', () => {
        ...
        it('POST returns 200 and token when login was successful', async() => {
            ...
            let userToSend = { name: 'user', password: 'password' } as User
            let token = ((await TestHelper.post('/api/login').send(userToSend).expect(200)).body as string[])[0]
            expect(token).not.to.be.empty
        })
        
    })

    describe.only('API register', () => {
        ...
        it('POST encrypts the password and returns 200 and login token when registration was successful', async() => {
            let password = 'password'
            let userToSend = { name: 'user', password: password } as User
            let token = ((await TestHelper.post('/api/register').send(userToSend).expect(200)).body as string[])[0]
            expect(token).not.to.be.empty
    ...
    describe('API User', () => {
        ...
        it('POST encrypts the password when it was sent', async() => {
        })
        ...
        it('PUT does not return the password of the user', async() => {
        })
