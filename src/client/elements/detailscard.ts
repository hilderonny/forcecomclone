...
import { DetailsSection, DetailsSectionConfig } from "./section";
...
export abstract class DetailsCard<T extends Type> extends Card {
    ...
    detailsSection: DetailsSection<T>;
    ...
    constructor(webApp: WebApp, id?: string, subUrl?: string) {
        ...
        let config = { } as DetailsSectionConfig<T>;
        if (id) {
            config.onSave = async (properties) => {
                console.log("TODO: SAVE");
            }
        } else {

        }
        self.detailsSection = new DetailsSection(config);
            // {
            // onAdd: async () => {
            ...
            // }
        // );
        self.addSection(self.detailsSection);
